<?php
/**
 * MySQL Standalone Database Class
 *
...
class MyDB {
	...
	*
	* Default behavior is to show errors.
	*
	* @since 0.71
	* @var bool
	*/
	...
	*/
	...
	public function prepare( $query, $args ) {
		...
		if ( strpos( $query, '%' ) === false ) {
			echo sprintf( 'The query argument of %s must have a placeholder.', 'wpdb::prepare()' );
		}
		...
		foreach ( $args as $arg ) {
			if ( ! is_scalar( $arg ) && ! is_null( $arg ) ) {
				echo sprintf( 'Unsupported value type (%s).', gettype( $arg ) );
			}
		}
		...
		if ( count( $args ) !== $placeholders ) {
			if ( 1 === $placeholders && $passed_as_array ) {
				...
				echo 'The query only expected one placeholder, but an array of multiple placeholders was sent.';
				return;
			} else {
				/*
	...
	*/
	...
	if ( $db_check_string ) {
		...
		if ( ! $row ) {
			throw new Exception("wpdb_strip_invalid_text_failure", 1);
		}
	...
	public function check_database_version() {
		...
		if ( version_compare( $this->db_version(), $required_mysql_version, '<' ) ) {
			...
			throw new Exception( sprintf( '<strong>ERROR</strong>: WordPress %1$s requires MySQL %2$s or higher', $wp_version, $required_mysql_version ) );
		}
	}
